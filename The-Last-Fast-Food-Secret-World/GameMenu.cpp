...
void GameMenu::Enter()
{
	...
	int x, y;
	x = 330, y = 195;
	btnManager.AddButton(make_shared<RectButton>("Stage", RECT{ x,y,x + 600,y + 75 }));
	...
	//상점 버튼
	y += 125;
	btnManager.AddButton(make_shared<RectButton>("Shop", RECT{ x,y,x + 600,y + 75 }));
	...
	x += 55;
	y += 250;
	btnManager.AddButton(make_shared<RectButton>("Lobby", RECT{ x,y,x + 485,y + 75 }));
...
void GameMenu::Logic()
{
	...
	if (btnManager.HandleClickId(mx, my) == "Stage") {
		STATE.ChangeState(GameState_M::E_InGameState::Stage);
	}
	if (btnManager.HandleClickId(mx, my) == "Shop") {
		STATE.ChangeState(GameState_M::E_InGameState::Shop);
	}
	if (btnManager.HandleClickId(mx, my) == "Lobby") {
