...
LPCTSTR lpszClass = TEXT("윤창민_25311033");					// 제목 표시줄에 표시
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT iMessage, WPARAM wParam, LPARAM lParam)
{
	...
	case WM_CREATE:
	{
		...
		//푸시버튼
		CreateWindow(TEXT("button"), TEXT("뽑기"), WS_CHILD | WS_VISIBLE		//(TEXT("button"), TEXT(버튼내용), WS_CHILD | WS_VISIBLE
	...
	case WM_LBUTTONDOWN:
		...
		{
			if (InCircle(450 ,635, mg.mx, mg.my))
			{
				gacha.GetGacha(TRUE, deck);
			}
			else if (InCircle(950, 635, mg.mx, mg.my))
			{
	...
	case WM_PAINT:
	{
		ImageTool* ab = new ImageTool;
		ab->AddObject(TEXT("asdf.jpg"), 50, 10, 197, 253);
		ab->AddObject(TEXT("asdf.jpg"), 300, 300, 197, 253);
		//ab->RemoveObject(TEXT("asdf.jpg"));
		hdc = BeginPaint(hWnd, &ps);

		ab->Render(hdc, rt, hWnd);
		if (screen == 0)
		{
